...
class PolymodInterpEx extends Interp
{
  ...
  private static var registeredScriptedClasses:Array<String> = [];
  ...
  public function exprWithType(e:Expr, ?t:CType):Dynamic
  {
    ...
    function getIdent(e:Expr):Null<String>
    {
      ...
      var importedClasses:Array<String> = [];
        ...
        for (decl in module)
        {
          switch (decl)
          {
            ...
            case DImport(path, _, name):
              ...
              if (!importedClasses.contains(importedClass.name))
              {
                Polymod.debug('Imported class ${importedClass.name} from ${importedClass.fullPath}');
                importedClasses.push(importedClass.name);
              }
            ...
            case DClass(c):
              ...
              if (extend != null)
              {
                ...
                if (imports.exists(superClassPath))
                {
                  ...
                  if (extendImport.cls == null) errorEx(EClassUnresolvedSuperclass(superClassPath, 'expected a class'));
